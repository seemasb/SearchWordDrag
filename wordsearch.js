...
class Puzzle {
  ...
  wordAt(position, direction, length) {
    ...
    console.log(length)   //when mobile length is not a number
...
class Selection {
  ...
  draw(ctx, wsize, hsize) {
    let colors = ['yellow', 'red', 'blue','black','green','grey','pink','yellow', 'red', 'blue','black']
...
class PuzzleApp extends App {
  ...
  renderList(parent) {
    this.puzzle.words.forEach(word => {
      ...
      li.setAttribute('class', 'ws-word badge badge-pill badge-light');
  ...
  touchup(x, y) {
    ...
    if (word) {
      ...
      // li.classList.add("badge badge-pill badge-light");
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  const wordLists = [
    ["believe", "brand", "colleague", "customer", "delight","experience","external","happy","product","supplier","value"]
  ]
  ...
  let puzzle = new Puzzle(12, 12, pick(wordLists), directions);
  puzzle.print();
